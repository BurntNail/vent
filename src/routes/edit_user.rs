...
use crate::{auth::{Auth, LoginDetails}, error::KnotError, liquid_utils::compile};
...
pub async fn post_edit_user (auth: Auth, State(pool): State<Arc<Pool<Postgres>>>, Form(LoginDetails { username, unhashed_password }): Form<LoginDetails>) -> Result<impl IntoResponse, KnotError> {
